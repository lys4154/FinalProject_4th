	        ...
	        <c:forEach var="fund" items="${ongoingFunded}" varStatus="out">
	            ...
	            <c:forEach var="project" items="${ongoingProject}" varStatus="inner">
	            	<c:if test="${out.index eq inner.index}">
		            	...
		            	<div class="pro_right">
			            	<div class="funded_day_seq"><span> 후원일 ${fund.fund_date}</span> ㅣ <span> 후원번호 ${fund.fund_seq}</span></div>
			            	<div class="long_title"><a href="/ongoing_detail/${fund.fund_seq }"> ${project.long_title } </a></div>
			            	<div> 번들명 넣어야 할 곳</div>
			            	<div> <span class="pay_status"> 후원 성공시 결제 예정일 .toLocalDate().plusDays(1) </span> </div>
        		...
        		<c:forEach var="project" items="${successProject}" varStatus="inner">
        			<c:if test="${out.index eq inner.index}">	
		            	...
		            	<div class="pro_right">
			            	...
			            	<div> 옵션넣을곳</div>
			            	<div> <span class="pay_status"> 후원 성공 toLocalDate().plusDays(1) 결제 완료</span> </div>
